...
%  [det_time,det_freq1,det_score]=tfMatchedFilterDet(v24,[100,340],10,3,0.25,250,75,0);
% 
% INPUTS 
%    y = demeaned & pressure corrected time series sampled at 24 kHz  
%    frange = is an a 1 x 2 vector with [minF1  maxF1] range of 
%             fundamental frequencies to consider (e.g., [100, 339]).    
...
%  The predicted frequency of first harmonic can be determined using 
%  water temperature. 
%  
%  If you want to turn this off, set the prediction and its uncertainity 
%  to the median of your frequency range. For example: 220, 130. 
% 
% [det_time,det_freq1,det_score]=tfMatchedFilterDet(v24,[100,339],10,3,0.25,220,120,0);
%
% OUTPUTS 
...
%
% If you don't have a GPU.   Comment gpuArray line out in the code 
%
% ToadFishFinder v1.1 
% Del Bohnenstiehl  
% June 2023 
...
for j=1:nkerns  % make each kernel
    % upper harmonic band 
fo=F2(1,j); f1=F2(2,j); % here, fo and f1 are start and stop freq of sweep 
...
k1(:,i)=(1-(x.^2)/(s.^2)).*exp(-(x.^2)/(2*s^2)); 
  ...
  % lower harmonic band  
fo=F1(1,j);  f1=F1(2,j);  % here, fo and f1 are start and stop freq of sweep 
...
% if plot = 1 make a plot of the weighting curve 
if ploton==1 
figure; plot(F1(1,:),tapper4cmatrix); title('Weighting'); ylabel('weight'); xlabel('Fo Frequency Hz'); 
end
...
title('percussive filter spectrgram with detections')
ylabel('Hz')
...
title('correlation with kernels'); ylabel('fo of kernel')
clim([-0.5,0.5]); 
%
ax(5)=subplot(5,1,5); hold on; 
for j=1:length(LOCS)
plot(gather(T(LOCS(j)))+cmatrix(:,LOCS(j)),F1(1,:));
title('correlation vs kernel freq for candidates')
xlabel('time');  ylabel('fo of kernel')
end
